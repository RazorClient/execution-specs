...
class ExecutionPayloadEnvelopeParis(Container):
    """
    The Paris `newPayload` envelope (consensus-specs Bellatrix).
    """
...
class ExecutionPayloadEnvelopeShanghai(Container):
    """
    The Shanghai `newPayload` envelope (consensus-specs Capella).
    """
...
class ExecutionPayloadEnvelopeAmsterdam(Container):
    """
    The Amsterdam `newPayload` envelope from PR #793.
    """
...
class PayloadAttributesShanghai(Container):
    """The Shanghai payload attributes (adds withdrawals)."""
...
class PayloadAttributesCancun(Container):
    """The Cancun payload attributes (adds parent_beacon_block_root)."""
...
class PayloadAttributesPrague(Container):
    """The Prague payload attributes (unchanged from Cancun)."""
...
class PayloadAttributesOsaka(Container):
    """The Osaka payload attributes (unchanged from Cancun)."""
...
class PayloadAttributesAmsterdam(Container):
    """The Amsterdam payload attributes (slot_number, target_gas_limit)."""
...
class ForkchoiceState(Container):
    """The forkchoice head/safe/finalized triple (fork-independent)."""
...
class ExecutionPayloadBodyCancun(Container):
    """The Cancun execution payload body (unchanged from Shanghai)."""
...
class ExecutionPayloadBodyPrague(Container):
    """The Prague execution payload body (unchanged from Shanghai)."""
...
class ExecutionPayloadBodyOsaka(Container):
    """The Osaka execution payload body (unchanged from Shanghai)."""
...
class ExecutionPayloadBodyAmsterdam(Container):
    """The Amsterdam execution payload body (adds block_access_list)."""
...
class BuiltPayloadOsaka(Container):
    """The Osaka getPayload response (blobs_bundle is V2)."""
...
class BlobsV2Request(Container):
    """The /blobs/v2 request (same shape as v1)."""

    versioned_hashes: List[VersionedHash, MAX_BLOBS_REQUEST]


class BlobV2Entry(Container):
    """One /blobs/v2 response entry (reused verbatim by /v3)."""
...
class BlobsV2Response(Container):
    """The /blobs/v2 response (all-or-nothing)."""

    entries: List[BlobV2Entry, MAX_BLOBS_REQUEST]


class BlobsV3Response(Container):
    """The /blobs/v3 response (reuses BlobV2Entry; only semantics differ)."""

    entries: List[BlobV2Entry, MAX_BLOBS_REQUEST]


class BlobsV4Request(Container):
    """The /blobs/v4 request (adds a cell-selection bitarray)."""
...
class ClientVersion(Container):
    """A client identity entry (JSON on the wire; SSZ shape)."""
