...
def test_is_changing_matches_upstream() -> None:
    """``random``/``one_count``/``max_count`` change; the rest are fixed."""
    changing = {
...
def test_same_seed_yields_identical_value(
    ...
    """A fixed seed reproduces the exact same value (the core guarantee)."""
...
def test_zero_mode_zeroes_scalars_and_byte_vectors() -> None:
    ...
    # empty -- emptiness is the job of ``nil_count``.
...
def test_max_count_fills_collections_to_cap() -> None:
    ...
    # ``extra_data`` capacity (32) is below the byte cap (48), so it fills to
